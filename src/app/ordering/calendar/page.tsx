...
import { Calendar, Plus, RefreshCw, Settings, RotateCcw, CalendarRange, ChevronLeft, ChevronRight } from 'lucide-react';
...
// Color map for assignees
const ASSIGNEE_COLORS: Record<string, string> = {
  'Heath': 'text-blue-600',
  'Jackie': 'text-purple-600',
  'Charlotte': 'text-pink-600',
  'Tosh': 'text-green-600',
  'Nathan': 'text-orange-600',
};
        ...
        <div className="flex items-center justify-between">
          <CardTitle className="text-lg flex items-center gap-2">
            ...
            <span className="text-sm font-normal text-gray-500">
              ({weekDates[0].toLocaleDateString('en-AU', { month: 'short', day: 'numeric' })} - {weekDates[6].toLocaleDateString('en-AU', { month: 'short', day: 'numeric' })})
            </span>
          </CardTitle>
                  ...
                  {orders.length === 0 ? (
                    ...
                    orders.map((order) => (
                      ...
                      >
                        ...
                        {order.orderDeadline && (
                          <div className="text-[10px] opacity-75">{order.orderDeadline}</div>
                        )}
                        {order.assignees && order.assignees.length > 0 && (
                          <div className="text-[10px] mt-0.5">
                            {order.assignees.map((name, i) => (
                              <span key={name} className={`${ASSIGNEE_COLORS[name] || 'text-gray-600'} font-medium`}>
                                {name}{i < order.assignees!.length - 1 ? ', ' : ''}
                              </span>
                            ))}
                          </div>
                        )}
...
export default function OrderCalendarPage() {
  ...
  return (
    <div className="container mx-auto py-8 px-4">
        ...
        <Card className="mb-4">
          <CardContent className="pt-4">
            <div className="flex items-center gap-4">
              <div className="w-64">
                <Label htmlFor="vendor-filter" className="text-sm">Filter by Vendor</Label>
                <Select
              ...
              {calendarData && (
                <div className="flex gap-4 text-sm text-gray-600">
                  <span>📅 {calendarData.summary.totalOrders} total</span>
                  <span className="text-gray-400">•</span>
                  <span>⏳ {calendarData.summary.upcoming} upcoming</span>
                  <span className="text-orange-500">⚡ {calendarData.summary.due} due</span>
                  <span className="text-green-500">✓ {calendarData.summary.placed} placed</span>
                </div>
              )}
            </div>
          </CardContent>
      ...
      {/* Calendar views - Both Weekly and Monthly */}
      {calendarData && (
